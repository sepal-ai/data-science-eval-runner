...
app = typer.Typer(help="Taiga - Data Science Agent Evaluation System")


@app.command()
def hello():
    """Original hello command."""
    print("Hello, World!")


@app.command()
def mcp():
    """Run MCP server for existing functionality."""
...
@app.command()
def ds_mcp():
    """Run Data Science Agent MCP server."""
    try:
        from src.ds_agent import create_ds_agent_mcp

        mcp_server = create_ds_agent_mcp()
...
@app.command()
def setup_data(db_path: str = typer.Option("/workdir/data.db", help="Database path")):
    """Setup mock data for evaluation."""
    try:
        from src.data_generator import setup_database_with_mock_data
...
def run_agent(
    ...
    db_path: str = typer.Option("/workdir/data.db", help="Database path"),
):
...
def list_problems(problems_dir: str = typer.Option("problems", help="Problems directory")):
    ...
    try:
        from src.cli_runner import load_problem_definitions
...
def validate_setup():
    ...
    expected_dirs = ["src", "problems", "taiga"]
